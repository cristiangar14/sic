<section class="nosotros-seccion1 contacto-seecion1">
    <div class="iconos-redes">
        <a href="https://co.linkedin.com/"><span class="fab fa-linkedin"></span></a>
        <a href="https://www.twitter.com/"><span class="fab fa-twitter-square"></span></a>
        <a href="https://www.facebook.com/"><span class="fab fa-facebook-square"></span></a>
    </div>
    <div class="nosotros-seccion1-banner contacto-seccion1-banner">
        <div class="nosotros-seccion1-banner-color contacto-seccion1-banner-color"></div>
    </div>
    <div class="nosotros-seccion1-cajainferior"></div>
    
</section>